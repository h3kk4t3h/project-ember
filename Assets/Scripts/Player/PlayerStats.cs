...
public class PlayerStats : CharacterStats
{
    ...
    private Dictionary<EquipmentSO, Vector4> equippedBonuses = new Dictionary<EquipmentSO, Vector4>();
    ...
    // ---------------------------
    // Lucas Things// Equipment Bonuses logic
    // ---------------------------

    public void AddEquipmentBonus(EquipmentSO eq)
    {
        if (eq == null || equippedBonuses.ContainsKey(eq)) return;
        var bonus = new Vector4(eq.bonusMaxHealth, eq.bonusMaxMana, eq.bonusSpellPower, eq.bonusArmor);
        equippedBonuses[eq] = bonus;
    ...
    {
        if (eq == null || !equippedBonuses.TryGetValue(eq, out var bonus)) return;
